  ...
  return (
    <div className='flex items-center justify-between px-8 py-3 border-b transition-colors'
      ...
      <Link href='/' className='relative text-3xl font-semibold' style={{ color: 'var(--text-primary)' }}>
        ...
        <span className='absolute text-[10px] font-bold -top-1 -right-10 px-2 py-0.5 rounded-full text-white bg-green-500'>Store</span>
      </Link>
      <div className='flex items-center gap-4'>
        <span className='text-sm hidden sm:block' style={{ color: 'var(--text-secondary)' }}>
          Hi, <span className='font-medium' style={{ color: 'var(--text-primary)' }}>{user?.name?.split(' ')[0] || 'Seller'}</span>
        </span>
