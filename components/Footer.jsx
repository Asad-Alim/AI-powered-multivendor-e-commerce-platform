...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            ...
            links: [
                ...
                { text: "asad@intellimart.dev", path: '/', icon: MailIcon },
    ...
    return (
        <footer className="mx-6" style={{ backgroundColor: 'var(--footer-bg)', color: 'var(--text-secondary)' }}>
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 text-sm" style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <div>
                        <Link href="/" className="text-4xl font-semibold" style={{ color: 'var(--text-primary)' }}>
                            <span style={{ color: 'var(--accent)' }}>intelli</span>mart<span style={{ color: 'var(--accent)', fontSize: '3rem', lineHeight: 0 }}>.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm" style={{ color: 'var(--text-secondary)' }}>
                            Welcome to IntelliMart — your AI-powered multivendor marketplace for the smartest gadgets and electronics. Discover top vendors, exclusive deals, and a seamless shopping experience.
                        </p>
                        <p className="mt-4 text-xs font-medium" style={{ color: 'var(--text-muted)' }}>Built by <span style={{ color: 'var(--accent)' }}>Asad</span> — Full Stack Developer</p>
                    </div>
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium md:mb-5 mb-3" style={{ color: 'var(--text-primary)' }}>{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="hover:underline transition" style={{ color: 'var(--text-secondary)' }}>{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
                    Copyright 2025 © IntelliMart by Asad. All Rights Reserved.
                </p>
            </div>
        </footer>
    );
